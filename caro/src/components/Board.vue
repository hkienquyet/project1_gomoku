...
function drawBackground(ctx, style, w, h, noShadow) {
  ...
  ctx.fillStyle = style.boardColorXO
...
function drawCoord(ctx, style, w, h, s, cs) {
  ...
  for (let i = 0; i < s; i++) {
    ctx.fillText(s - i, 0, cs * i + cs / 2, paddingX)
  }
  ...
  for (let i = 0; i < s; i++) {
    ctx.fillText(String.fromCharCode('A'.charCodeAt(0) + i), cs * i + cs / 2, 0)
  }
...
function drawPiece(ctx, style, cs, position, end) {
  // Giảm kích thước quân cờ
  let radius = style.pieceScale / 3 // Điều chỉnh kích thước nhỏ hơn
  ctx.save()
  ctx.translate(paddingX + cs, paddingTop + cs) // Điều chỉnh vị trí
  ctx.scale(cs, cs)

  ctx.lineWidth = style.pieceStrokeWidthXO
  ctx.fillStyle = style.boardColorXO
  ctx.strokeStyle = style.pieceStrokeX
  ...
  for (let i = 0; i < end; i += 2) {
    ...
    // Vẽ quân cờ X
    ctx.beginPath()
    ctx.moveTo(pos[0] - radius, pos[1] - radius) // Đường chéo trái
    ctx.lineTo(pos[0] + radius, pos[1] + radius) // Đường chéo phải
    ctx.moveTo(pos[0] + radius, pos[1] - radius) // Đường chéo phải
    ctx.lineTo(pos[0] - radius, pos[1] + radius) // Đường chéo trái
    ctx.stroke() // Vẽ đường viền

    // Nếu bạn muốn có đường viền, hãy kiểm tra và sử dụng stroke
    if (style.pieceStrokeWidth > 0) {
      ctx.stroke() // Vẽ đường viền
    }
  }

  // Vẽ quân cờ O
  ctx.strokeStyle = style.pieceStrokeO
  for (let i = 1; i < end; i += 2) {
    let pos = position[i]
    fillCircle(ctx, pos[0], pos[1], radius) // Giữ nguyên kích thước nhỏ hơn cho quân O
...
function drawIndex(ctx, style, cs, position, end, startIndex, highlight) {
  ctx.save()
  ctx.font = style.indexFontStyle + ' ' + style.indexScaleXO * cs + 'px ' + style.indexFontFamily
  ctx.textAlign = 'center'
  ctx.textBaseline = 'middle'
  ctx.translate(paddingX + cs / 2, paddingTop + cs / 2)
  ...
  for (let i = startIndex; i < end - 1; i += 1) {
    ...
    ctx.fillStyle = style.boardColorXO
    fillCircle(ctx, cs * pos[0] + cs / 2, cs * pos[1] + cs / 2, style.indexScaleXO * cs)
    ctx.fillStyle = i % 2 == 0 ? style.indexColorX : style.indexColorO
    ctx.fillText(text, cs * pos[0] + cs / 2, cs * pos[1] + cs / 2, cs)
  }

  if (end > startIndex) {
    let pos = position[end - 1]
    if (highlight) ctx.fillStyle = style.lastStepColor
    else ctx.fillStyle = end % 2 == 1 ? style.indexColorO : style.indexColorX
    ctx.fillText(end - startIndex, cs * pos[0] + cs / 2, cs * pos[1] + cs / 2, cs)
  }
...
function drawLastStep(ctx, style, cs, position, end) {
  ...
  // Tính toán vị trí để vẽ hình chữ nhật
  ctx.fillRect(pos[0], pos[1], style.winlineWidth * 1.2, style.winlineWidth * 1.2)
...
function drawWinline(ctx, style, cs, winline) {
  ...
  ctx.translate(paddingX + cs, paddingTop + cs)
  ctx.scale(cs, cs)
...
function drawRealtime(ctx, style, cs, moves) {
  ...
  for (let p of moves.thought) {
    fillCircle(ctx, p[0] + cs / 2, p[1] + cs / 2, style.realtimeMoveScale)
  }
  ...
  for (let p of moves.best) {
    fillCircle(ctx, p[0] + cs / 2, p[1] + cs / 2, style.bestMoveScale)
  }
...
export default {
  ...
  methods: {
    drawBoardLayer(ctx, noclear) {
      ...
      let cellSize = this.boardWidth / (this.boardSize + 1)
    ...
    onMouseDown(event) {
      ...
      let cellSize = this.boardWidth / (this.boardSize + 1)
      x = Math.floor((x - paddingX - cellSize / 2) / cellSize)
      y = Math.floor((y - paddingTop - cellSize / 2) / cellSize)
