<template>
  <div class="game">
    ...
    <div class="info-box" :style="showAnalysis ? {} : { display: 'none' }">
      <div>
        <popup-radio
          :title="$t('setting.board.boardTheme.title')"
          v-model="boardThemeValue"
      ...
      <div class="container">
        <div class="columns">
          <div class="column black">
            <span>Quân đen (X)</span>
          </div>
          <div class="column white">
            <span>Quân trắng (O)</span>
          </div>
        </div>
        <div class="columns">
          <div class="column black">
            <div class="switch-container">
              <label class="switch">
                <input type="checkbox" v-model="aiThinkBlackValue" />
                <span class="slider"></span>
              </label>
              <span>{{ aiThinkBlack ? 'AI' : 'Người' }}</span>
            </div>
          </div>
          <div class="column white">
            <div class="switch-container">
              <label class="switch">
                <input type="checkbox" v-model="aiThinkWhiteValue" />
                <span class="slider"></span>
              </label>
              <span>{{ aiThinkWhite ? 'AI' : 'Người' }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="aiThinkBlack || aiThinkWhite">
        <x-progress
        ...
        <!-- Thông tin đầu ra của phân tích điểm đơn -->
        <x-table
          v-if="nbest == 1"
          :cell-bordered="true"
        ...
        <!-- Thông tin đầu ra của phân tích đa điểm -->
        <div v-else>
...
import BoardClassic from '@/components/BoardClassic.vue'
...
export default {
  ...
  computed: {
    ...
    boardThemeValue: {
      ...
      set(v) {
        this.setValue({ key: 'boardTheme', value: v })
      },
    },
    boardThemeOptions() {
      return [
        { key: 0, value: this.$t('setting.board.boardTheme.XO') },
        { key: 1, value: this.$t('setting.board.boardTheme.classic') },
      ]
    },
    aiThinkBlackValue: {
      get() {
    ...
    currentBoard() {
      return this.boardTheme === 0 ? 'Board' : 'BoardClassic'
    },
    ...
    startThinkBalance(mode) {
      ...
      this.think({ balanceMode: mode }).then((pos) => {
        ...
        if (mode == 2) {
          ...
          pos2 = [pos2[0], this.boardSize - 1 - pos2[1]] // Lật trục y
...
.switch {
  ...
  width: 60px;
  height: 34px;
  margin-right: 10px; /* Khoảng cách giữa switch và nhãn */
}
...
.slider {
  ...
  transition: 0.4s;
  border-radius: 34px; /* Bo tròn cho slider */
}
...
.slider:before {
  ...
  transition: 0.4s;
  border-radius: 50%; /* Bo tròn cho nút trên slider */
}
...
input:checked + .slider:before {
  transform: translateX(26px); /* Di chuyển nút sang bên phải khi bật */
}
